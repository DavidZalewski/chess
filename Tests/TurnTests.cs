...
namespace Tests
{
    public class TurnTests
    {
        ...
        [Test]
        public void Test_ConstructTurn_Capture_Piece_EnPassant_AlreadyOnEnPassantRank_Success()
        {
            ChessBoard board = new();
            List<ChessPiece> chessPieces = ChessPieceFactory.CreateChessPieces();
            // find white pawn 1
            ChessPiece whitePawn = chessPieces.First(pieces => pieces.GetColor() == ChessPiece.Color.WHITE &&
                                                     pieces.GetPiece() == ChessPiece.Piece.PAWN);
            // find black pawn 4
            ChessPiece blackPawn4 = chessPieces.First(pieces => pieces.GetColor() == ChessPiece.Color.BLACK &&
                                         pieces.GetPiece() == ChessPiece.Piece.PAWN && pieces.GetId() == 4);

            GameController gc = new(board, chessPieces);
            gc.StartGame();

            // set white pawn 1 to A4 and black pawn 4 to B4 to setup en passant
            whitePawn.Move(board, new("A4"));
            blackPawn4.Move(board, new("B4"));

            // move black pawn 4 to C4
            blackPawn4.Move(board, new("C4"));

            BoardPosition newPosition = new("B5");
            Assert.That(whitePawn.IsValidMove(board, newPosition), Is.True);
            ...
            Assert.Multiple(() =>
            {
                ...
                ChessPiece removedBlackPawn4Piece = removedChessPieces.First();
                Assert.That(removedBlackPawn4Piece.GetPiece() == ChessPiece.Piece.PAWN &&
                            removedBlackPawn4Piece.GetColor() == ChessPiece.Color.BLACK &&
                            removedBlackPawn4Piece.GetId() == 4);
            });

        }

        [Test]
        public void Test_ConstructTurn_Capture_Piece_EnPassant_DifferentPosition2_Success()
        {
            ChessBoard board = new();
            List<ChessPiece> chessPieces = ChessPieceFactory.CreateChessPieces();
            // find white pawn 3
            ...
            // set white pawn 3 to C4 and black pawn 4 to D4 to setup en passant
            whitePawn.Move(board, new("C4"));
            blackPawn4.Move(board, new("D4"));
            (blackPawn4 as ChessPiecePawn).SetMovedTwoSquares();

            BoardPosition newPosition = new("D5");
            Assert.That(whitePawn.IsValidMove(board, newPosition), Is.True);
        ...
        [Test]
        public void Test_ConstructTurn_SetupEnPassant_NextMove_Success()
        {
            ChessBoard board = new();
            List<ChessPiece> chessPieces = ChessPieceFactory.CreateChessPieces();
            // find white pawn 5
            ChessPiece whitePawn = chessPieces.First(pieces => pieces.GetColor() == ChessPiece.Color.WHITE &&
                                                     pieces.GetPiece() == ChessPiece.Piece.PAWN && pieces.GetId() == 5);
            // find black pawn 6
            ChessPiece blackPawn6 = chessPieces.First(pieces => pieces.GetColor() == ChessPiece.Color.BLACK &&
                                         pieces.GetPiece() == ChessPiece.Piece.PAWN && pieces.GetId() == 6);

            GameController gc = new(board, chessPieces);
            gc.StartGame();

            // set white pawn 5 to E5
            whitePawn.Move(board, new("E5"));

            // setup en passant by moving black pawn 6 next to white pawn 5
            blackPawn6.Move(board, new("F5"));

            BoardPosition newPosition = new("F6");
            Assert.That(whitePawn.IsValidMove(board, newPosition), Is.True);
            ...
            Assert.Multiple(() =>
            {
                ...
                ChessPiece removedBlackPawn6Piece = removedChessPieces.First();
                Assert.That(removedBlackPawn6Piece.GetPiece() == ChessPiece.Piece.PAWN &&
                            removedBlackPawn6Piece.GetColor() == ChessPiece.Color.BLACK &&
                            removedBlackPawn6Piece.GetId() == 6);
            });
        }
    }
}
